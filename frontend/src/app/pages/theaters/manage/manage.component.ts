...
export class ManageComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute,
    ...
    this.trySend = false;
    this.theater = { id: 0 };
    this.configFormGroup() // Es donde se van a empezar a mezclar el FormBuilder con el validaro de clase, defi
  }

  ngOnInit(): void {
    const currentUrl = this.activatedRoute.snapshot.url.join('/'); // Estamos tomando una foto a la ruta y se esta partiendo por slashes
    ...
    if (this.activatedRoute.snapshot.params.id) { // es .id, porque en el routing le pusimos :id
  ...
  getTheater(id: number) {
    this.theatersService.view(id).subscribe({
      next: (response) => {
        ...
        this.theFormGroup.patchValue({ // Estas son las lineas que llenan el formulario con los datos que vienen del servidor
          id: this.theater.id,
